...
class custom_PCA:
    ...
    def train(self,training_data):
        ...
        for i in range(len(training_data)):
            ...
            self.number_elements+=1
            tmp.add_data(np.array([float(training_data[i][1]),float(training_data[i][2])]))
        tmp.calculate_mean_vector()
        tmp.calculate_covariance_matrix()
        tmpmat = tmp.cov_mat
        # Call functions for calculating means and standard deviations
        for x in self.classes:
            self.classes[x].calculate_mean_vector()
            self.classes[x].cov_mat = tmpmat
            self.classes[x].prior = self.classes[x].number_elements/self.number_elements
